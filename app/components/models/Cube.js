...
export default function Cube({MIDIdata, props}) {
  ...
  useEffect(() => {
    if (MIDIdata.id == 0) {
        if (MIDIdata.val > 0) {
            setcubeData((prev) => ({ ...prev, w: MIDIdata.val/7.11 }))
        }
    } else if (MIDIdata.id == 1) {
        if (MIDIdata.val > 0) {
            setcubeData((prev) => ({ ...prev, h: MIDIdata.val/7.11 }))
        }
    } else if (MIDIdata.id == 2) {
        if (MIDIdata.val > 0) {
            setcubeData((prev) => ({ ...prev, d: MIDIdata.val/7.11 }))
        }
    }
  ...
  useEffect(() => {
    ...
    if (cube.current.geometry) {
        const geometry = new THREE.BoxGeometry(minSize + cubeData.w, minSize + cubeData.d, minSize + cubeData.h)
  ...
  return (
    <>
      <mesh {...props} ref={mesh}>
        <boxGeometry ref={cube} args={[minSize + cubeData.w, minSize + cubeData.d, minSize + cubeData.h]}/>
